...
def pixel_weight_matrix(image_path, weight_path):
    ...
    np.savetxt("iterative_sample_pixel_weight_matrix_top1_r_0_33.txt", channel_r, fmt="%.10f", delimiter=" ")
    np.savetxt("iterative_sample_pixel_weight_matrix_top1_g_0_33.txt", channel_g, fmt="%.10f", delimiter=" ")
    np.savetxt("iterative_sample_pixel_weight_matrix_top1_b_0_33.txt", channel_b, fmt="%.10f", delimiter=" ")
...
for image_file in image_files:
    ...
    while flag == 0:
        ...
        pixel_weight_matrix_R_iterative = "iterative_sample_pixel_weight_matrix_top1_r_0_33.txt"
        pixel_weight_matrix_G_iterative = "iterative_sample_pixel_weight_matrix_top1_g_0_33.txt"
        pixel_weight_matrix_B_iterative = "iterative_sample_pixel_weight_matrix_top1_b_0_33.txt"
            ...
            for j in range(224):
                ...
                if matrix_R_data_transform[i][j] > 0 and pixel_weight_matrix_R_iterative[i][j] > 0 and ~(
                    ...
                    change_matrix_R[i][j] = np.clip(change_matrix_R[i][j],
                                                    actual_image_channel_R[i][j] - 255 * step,
                                                    actual_image_channel_R[i][j] + 255 * step).astype(np.uint8)
                    change_matrix_R[i][j] = np.clip(change_matrix_R[i][j], 0, 255).astype(np.uint8)
                ...
                if matrix_G_data_transform[i][j] > 0 and pixel_weight_matrix_G_iterative[i][j] > 0 and ~(
                    ...
                    change_matrix_G[i][j] = np.clip(change_matrix_G[i][j],
                                                    actual_image_channel_G[i][j] - 255 * step,
                                                    actual_image_channel_G[i][j] + 255 * step).astype(np.uint8)
                    change_matrix_G[i][j] = np.clip(change_matrix_G[i][j], 0, 255).astype(np.uint8)
                ...
                if matrix_B_data_transform[i][j] > 0 and pixel_weight_matrix_B_iterative[i][j] > 0 and ~(
                    ...
                    change_matrix_B[i][j] = np.clip(change_matrix_B[i][j],
                                                    actual_image_channel_B[i][j] - 255 * step,
                                                    actual_image_channel_B[i][j] + 255 * step).astype(np.uint8)
                    change_matrix_B[i][j] = np.clip(change_matrix_B[i][j], 0, 255).astype(np.uint8)
        ...
        image_pil.save("adversarial_samples_with_reduced_background_classification_contribution_value_0_33.png")

        # Create model
        model = AlexNet(num_classes=3).to(device)
        model.load_state_dict(torch.load(the_absolute_path_of_the_weight_file))

        iterative_image_path = "adversarial_samples_with_reduced_background_classification_contribution_value_0_33.png"
        ...
        if iterative_image_index != actual_image_index:
            print("success")
            current_image_path = "adversarial_samples_with_reduced_background_classification_contribution_value_0_33.png"
            ...
            print(fail_images)
            file_path = "fail_images_0_33.txt"
