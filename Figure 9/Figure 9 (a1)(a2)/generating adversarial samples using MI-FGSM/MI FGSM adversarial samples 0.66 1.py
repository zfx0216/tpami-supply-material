...
weights_path = "..."  # Update your model weights path
        ...
        for j in range(224):
            if adv_image_channel_R[i][j] > actual_image_channel_R[i][j]:
                adv_image_channel_R[i][j] = np.clip(adv_image_channel_R[i][j],
                                                    ...
                                                    actual_image_channel_R[i][j] + 255 * ratio * 3).astype(np.uint8)
            else:
                adv_image_channel_R[i][j] = np.clip(adv_image_channel_R[i][j],
                                                    actual_image_channel_R[i][j] - 255 * ratio * 3,
            ...
            if adv_image_channel_G[i][j] > actual_image_channel_G[i][j]:
                adv_image_channel_G[i][j] = np.clip(adv_image_channel_G[i][j],
                                                    ...
                                                    actual_image_channel_G[i][j] + 255 * ratio * 3).astype(np.uint8)
            else:
                adv_image_channel_G[i][j] = np.clip(adv_image_channel_G[i][j],
                                                    actual_image_channel_G[i][j] - 255 * ratio * 3,
            ...
            if adv_image_channel_B[i][j] > actual_image_channel_B[i][j]:
                adv_image_channel_B[i][j] = np.clip(adv_image_channel_B[i][j],
                                                    ...
                                                    actual_image_channel_B[i][j] + 255 * ratio * 3).astype(np.uint8)
            else:
                adv_image_channel_B[i][j] = np.clip(adv_image_channel_B[i][j],
                                                    actual_image_channel_B[i][j] - 255 * ratio * 3,
