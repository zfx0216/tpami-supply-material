"""
The main function of this code file is to evaluate the scores of experimental images generated based on AlexNet that aim to decrease classification values.
First, you need to import the AlexNet model using `from guid_Xmage.experiment_5.AlexNet.model_AlexNet import AlexNet`.
Second, you need to set the parameter `weights_absolute_path` to the absolute path of the weight file of the officially trained AlexNet model.
...
# Instantiate the AlexNet model and move it to the device
...
for img_path_actual, img_path_tampere in zip(image_paths_actual, tampered_images):
    ...
    alexnet.eval()
    with torch.no_grad():
        output_actual = alexnet(input_batch_actual)
    ...
    alexnet.eval()
    with torch.no_grad():
        output_tampere = alexnet(input_batch_tampere)
