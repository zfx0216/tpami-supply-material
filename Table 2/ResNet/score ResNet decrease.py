"""
The main function of this code file is to evaluate the scores of experimental images generated based on ResNet that aim to decrease classification values.
First, you need to set the parameter `weights_absolute_path` to the absolute path of the weight file of the officially trained ResNet model.
Second, you need to set the parameter `json_absolute_path` to the absolute path of the index file containing the 1000 classification labels of the model.
Third, you need to set the parameter `actual_image_folder_absolute_path` to the absolute path of the folder storing the original images.
Fourth, you need to set the parameter `tampered_image_absolute_path` to the absolute path of the folder storing the tampered images.
...
# Instantiate the ResNet model and move it to the device
resnet = models.resnet18(num_classes=1000).to(device)

# Load the model weights
weights_absolute_path = "..."
assert os.path.exists(weights_absolute_path), "File '{}' does not exist.".format(weights_absolute_path)
resnet.load_state_dict(torch.load(weights_absolute_path, map_location=device))
...
for img_path_actual, img_path_tampere in zip(image_paths_actual, tampered_images):
    ...
    resnet.eval()
    with torch.no_grad():
        output_actual = resnet(input_batch_actual)
    ...
    resnet.eval()
    with torch.no_grad():
        output_tampere = resnet(input_batch_tampere)
